            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    {% for i in range(block.headers|length) %}
                        {# Safe conversion logic to prevent 500 error on empty strings #}
                        {% set actual = (block.gmts_qty[i]|replace(',', '') or 0)|int %}
                        {% set qty_3 = (actual * 1.03)|round|int %}
                        {% set cut_qc = 0 %}
                        {% if i < block.cutting_qc|length %}
                            {% set cut_qc = (block.cutting_qc[i]|replace(',', '') or 0)|int %}
                        {% endif %}
                        {% set inp_qty = 0 %}
                        {% if i < block.sewing_input|length %}
                            {% set inp_qty = (block.sewing_input[i]|replace(',', '') or 0)|int %}
                        {% endif %}
...
# --- NEW: Accessories Challan Template ---
...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            .challan-container { border: none; box-shadow: none; padding: 0; }
    ...
    <div class="container">
        ...
        <div class="challan-container">
            ...
            <div class="info-grid">
                ...
                <div class="info-box">
                    <div class="info-label">Booking Ref No</div>
                    <div class="info-val">{{ ref_no }}</div>
                </div>
                <div class="info-box">
                    <div class="info-label">Buyer</div>
                    <div class="info-val">{{ buyer }}</div>
                </div>
                <div class="info-box">
                    <div class="info-label">Style No</div>
                    <div class="info-val">{{ style }}</div>
                </div>
                <div class="info-box">
                    <div class="info-label">Sewing Line</div>
                    <div class="info-val">Line # {{ line_no }}</div>
                </div>
            </div>
            ...
            <div class="signatures">
                <div class="sig-block">Store Incharge</div>
                <div class="sig-block">Receiver Signature</div>
                <div class="sig-block">Authorized By</div>
            </div>
            
            <div style="text-align: center; margin-top: 40px; font-size: 12px; color: #aaa;">
                System Generated Report | Created By Mehedi Hasan
            </div>
        </div>
    </div>
</body>
...
<body>
            ...
            <ul class="nav-menu">
                ...
                <li class="nav-item">
                    <a class="nav-link" onclick="showSection('accessories', this)">
                        <i class="fas fa-tags"></i> Accessories Challan
                    </a>
                </li>
                 <li class="nav-item">
        ...
        <div class="admin-content">
            ...
            <div class="work-area" id="work-area">
                ...
                <div id="accessories-section" class="work-section" style="display:none; width: 100%; max-width: 600px;">
                    <div class="glass-card" style="background: rgba(255,255,255,0.05); box-shadow: none; border: none;">
                        <h2 style="margin-bottom: 20px; font-weight: 500;"><i class="fas fa-tags"></i> Accessories Challan</h2>
                        
                        <div id="step1-booking">
                            <div class="input-group">
                                <label for="challan_ref">Booking Reference No</label>
                                <input type="text" id="challan_ref" placeholder="Enter Booking No (e.g. DFL/24/..)" required>
                            </div>
                            <button onclick="fetchBookingColors()" style="background: #e67e22;">Fetch Colors</button>
                        </div>

                        <div id="step2-details" style="display:none; margin-top: 20px; border-top: 1px solid rgba(255,255,255,0.2); padding-top: 20px;">
                            <form action="/generate-accessories-challan" method="post" target="_blank">
                                <input type="hidden" name="ref_no" id="hidden_ref_no">
                                <input type="hidden" name="buyer" id="hidden_buyer">
                                <input type="hidden" name="style" id="hidden_style">
                                
                                <div class="input-group">
                                    <label>Select Color</label>
                                    <div id="color-options" style="display: flex; flex-wrap: wrap; gap: 10px;">
                                        </div>
                                </div>
                                
                                <div class="input-group">
                                    <label for="line_no">Sewing Line No</label>
                                    <input type="text" name="line_no" id="line_no" placeholder="e.g. 15" required>
                                </div>
                                
                                <div class="input-group">
                                    <label for="qty">Label Quantity (Pcs)</label>
                                    <input type="number" name="qty" id="qty" placeholder="e.g. 500" required>
                                </div>
    ...
    <script>
        // --- Accessories Challan Logic ---
        function fetchBookingColors() {{
            const refNo = document.getElementById('challan_ref').value;
            if (!refNo) {{ swal("Error", "Please enter a booking number", "error"); return; }}
            
            const overlay = document.getElementById('loading-overlay');
            overlay.style.display = 'flex';
            
            fetch('/admin/get-booking-colors', {{
                method: 'POST',
            ...
            .then(data => {{
                ...
                if (data.status === 'success') {{
                    ...
                    document.getElementById('hidden_ref_no').value = refNo;
                    document.getElementById('hidden_buyer').value = data.meta.buyer;
                    document.getElementById('hidden_style').value = data.meta.style;
                    
                    const colorDiv = document.getElementById('color-options');
                    colorDiv.innerHTML = '';
                    data.colors.forEach(color => {{
                        colorDiv.innerHTML += `
                            <label style="background:rgba(255,255,255,0.1); padding:5px 10px; border-radius:5px; cursor:pointer; display:flex; align-items:center; gap:5px; border:1px solid rgba(255,255,255,0.2);">
...
def get_booking_colors():
    ...
    # Extract Colors
    colors = [block.get('color') for block in report_data if block.get('color')]
    unique_colors = list(set(colors))
    
    # Extract Meta (Assuming first block has valid meta like Buyer/Style)
    meta = {
        'buyer': report_data[0].get('buyer', 'N/A') if report_data else 'N/A',
        'style': report_data[0].get('style', 'N/A') if report_data else 'N/A'
    }
    
    return jsonify({'status': 'success', 'colors': unique_colors, 'meta': meta})
...
def generate_accessories_challan():
    ...
    return render_template_string(ACCESSORIES_CHALLAN_TEMPLATE, 
                                  ref_no=ref_no, buyer=buyer, style=style, 
                                  color=color, line_no=line_no, qty=qty, date=current_date)
